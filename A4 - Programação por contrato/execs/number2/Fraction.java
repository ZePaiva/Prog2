...
public class Fraction implements Comparable<Fraction>{
  ...
  public Fraction(int numero, int denumero) {
    ...
    while (n){
      if (Math.abs(denumero) > Math.abs(numero)) {
        int fact = mdc(Math.abs(denumero), Math.abs(numero));
        if (fact == Math.abs(denumero)) {
          break;
        }
        denumero /= fact;
        numero /= fact;
      } else if (Math.abs(denumero) < Math.abs(numero)) {
        int fact = mdc(Math.abs(numero), Math.abs(denumero));
        if (fact == Math.abs(numero)) {
          break;
        }
        denumero /= fact;
        numero /= fact;
  ...
  //Função do comparação 
  public int compareTo(Fraction b) {
    double fr1 = num/den;
    double fr2 = b.num/b.den;
