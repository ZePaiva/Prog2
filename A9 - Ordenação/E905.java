...
public class E905 {
	...
	public static void main(String[] args) throws IOException /* Note that the Program is not robust!*/{
		...
		File listeds = new File("words.txt");	
		String[][] wordsi;
		if (listeds.exists() && !listeds.isDirectory() && listeds.canRead()){
			wordsi = new String[args.length + 1][];
			wordsi[args.length] = extractsWords(listeds);
		} else {
			wordsi = new String[args.length][];
		}
