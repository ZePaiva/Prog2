...
public class HashTable<E> {
	...
	// função que retorna as chaves no dicionário
	// para não ter de fazer recursivamente utilizou-se o método do array
	public String[] keys(){
		// array das chaves
		...
		for(int i = 0; i < array.length; i++) {
				// chaves de cada lista (KeyValueList)
				String[] secKeys = array[i].keys();

				for(int j = 0; j < secKeys.length; j++) {
						// colocar no array principal
						chaves[keysAmount]= secKeys[j];
						keysAmount++;
				}
		}
	...
	// retorna o dicionário no formato de uma só str
	public String	toString(){
		// string onde vai ser colocado o dicionário
		String dict = "";
		// string da vírgula, originalmente nãoe é nada senão fica com uma vírgula no início estranha
		String virg = "";
